...
A token is what is provided by a server that enables a server to authenticate  a user that tries to log into the application. 


- [ ] What steps can you take in your web apps to keep your data secure?

step1 - When user logs in, token is generated by the server
step2 - Save this token in localStorage
step3- Get the token from localStorage and pass it to component with data that's need to be secure.
step4 - The server checks whether the token is the same and if it is then user can access data.
step5- Once user has finished with the data, he or she can log out and once they log out the data is removed from local storage.

- [ ] Describe how web servers work.

A web server uses HTTP to serve files to users. It does this by getting a request from the client then returns the response to the client. Example of HTTP request can be get- to receive data from server, post- to add data to server, put - to update data on server and delete- to remove data from server.
